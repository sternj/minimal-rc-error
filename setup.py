...
class BuildExtCommand(setuptools.command.build_ext.build_ext):
    ...
    def build_libscalene(self, arch_flags):
        scalene_temp = path.join(self.build_temp, 'force_rc_error')
        scalene_lib = path.join(self.build_lib, 'force_rc_error')
    ...
    def copy_extensions_to_source(self):
        ...
        if sys.platform != 'win32':
            scalene_lib = path.join(self.build_lib, 'force_rc_error')
            inplace_dir = self.get_finalized_command('build_py').get_package_dir('force_rc_error')
            libscalene = 'libscalene' + dll_suffix()
            self.copy_file(path.join(scalene_lib, libscalene),
                           path.join(inplace_dir, libscalene))


pywhere = Extension('force_rc_error.pywhere',
...
setup(
    ...
    entry_points={"console_scripts": ["force_rc_error = force_rc_error.__main__:main"]},
    python_requires=">=3.8",
)
